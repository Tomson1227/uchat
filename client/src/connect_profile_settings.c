...
void show_log_out_window(GtkButton *btn, t_chat *chat) {
    ...
    GtkButton *btn_confirm_log_out = gtk_builder_get_object(chat->builder, "btn_confirm_log_out");
    GtkButton *btn_cancel_log_out = gtk_builder_get_object(chat->builder, "btn_cancel_log_out");
    
    g_signal_connect(btn_confirm_log_out, "clicked", G_CALLBACK(log_out), chat);
    g_signal_connect(btn_cancel_log_out, "clicked", G_CALLBACK(cancel_log_out), log_out);
